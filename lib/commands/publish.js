...
function showResults(results) {
  ...
  const usedWidth = 26; // Status columns + padding
...
async function publishArticles(files, options) {
  ...
  try {
    ...
    if (articles.length === 0) {
      console.warn(chalk`No articles to publish.`);
      return;
    }
    ...
    const spinner = ora({
      color: 'green',
      spinner: 'point',
      text: 'Retrieving articles from dev.to…'
    }).start();
    ...
    spinner.text = 'Publishing articles to dev.to…';
